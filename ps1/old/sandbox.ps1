...
if ($cmdOutput.length -lt 4) {
	...
	Add-Content DockerFile "apt-get -yq install openssh-client openssl curl sudo unzip wget vim git docker-compose && \"
	Add-Content DockerFile "apt-get -yq autoremove"
   	Add-Content DockerFile "RUN cd /tmp && wget --progress=bar:force --no-check-certificate https://releases.hashicorp.com/terraform/${TERRAFORM_VER}/terraform_${TERRAFORM_VER}_linux_amd64.zip && \"
...
write-host "  packages: openssh-client openssl curl sudo unzip wget vim git docker-compose terraform"
...
# Modify the data volumes to your needs.
# sudo will run passwordless.
